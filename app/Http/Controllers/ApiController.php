...
class ApiController extends Controller
{
    ...
    public function export_trouble_ticket(Request $request)
    {
        ...
        if (str_contains(session()->get('user')->privilege, 'USER')) {
            ...
            if (count($explode) == 0) {
                ...
                $query->where(function ($q) use ($explode) {
                    foreach ($explode as $key => $e) {
                        if ($key == 0) $q->where('mitra_om', $e);
                        else $q->orWhere('mitra_om', $e);
                    }
                });
            }
        }

        // Display headers (pretty)
        $headers1 = [
            "Status","Name","Docstatus","Aging","Severity","PID","Tower Owner","Portofolio","Region","Category",
            "TT Description","Area","Address","Vendor Mitra OM","Timestamp Closed","Customer","Tower Name","PIC",
            "Timestamp Open","TT ID Tenant","Tenant ID","SiteID from Tenant","Tanggal Request ANT","Timestamp Resolved",
            "Service Level Agreement","Service Level Agreement Status","Actual Category","TT From","TT ID AmpuhC",
            ...
            "Timestamp Departure","Timestamp Arrived","Detail Issue Type","Closed By", "Severity Mtel", "Detail SLA Status",
        ];

        // Column keys to fetch from DB
        $headers = [
            "status","name","docstatus","aging","priority","pid","tower_owner","portofolio","region","issue_type",
            "tt_description","area","address","mitra_om","ts_closed","customer","tower_name","pic","ts_open",
            "tt_ant_id","tenant_id","sid_tenant","tanggal_request_ant","ts_resolved","service_level_agreement",
            ...
            "ts_on_progress","ts_pickup","ts_departure","ts_arrived","detail_issue_type","closed_by", "severity_mtel", "detail_sla_status",
        ];

        // Fetch rows
        $rows = $query->get($headers);

        // Build spreadsheet
        $spreadsheet = new Spreadsheet();
        $sheet       = $spreadsheet->getActiveSheet();

        // 1) Write header row (A1…)
        $sheet->fromArray($headers1, null, 'A1');

        // 2) Write data rows explicitly as TEXT so values stay exactly as-is
        $startRow = 2;
        $rowIndex = $startRow;
        ...
        // 3) Force text format on all populated cells as an extra guard
    ...
    public function export_maintenance_order(Request $request)
    {
        ...
        $headers1 = [
            "Status","Name","Docstatus","Expired Visit","Site ID","Return Notes","Schedule Visit",
            "Timestamp Submitted by Vendor","Vendor Mitra OM","Tower Name","Scope of Work","Fullname Engineer",
            "Timestamp Ready to Execute","Timestamp End","Timestamp Approved by Vendor","Timestamp Execution",
            "Timestamp Open","Timestamp Need Assign","Timestamp Approved by SM/KSM","Timestamp Expired","MO ID",
            "pdf_link","Regional","PID","TenantID","Customer","Engineer","District or City","Area","Batch","Year",
            "Funcloc","Site ID Tenant","MO Type","Type","Visitor Permit","Province","Address","Maintenance Zone",
            "Lat Long","Kode Tipe Site","Portofolio","Return Schedule Notes","Timestamp Request Schedule",
            "Order Numner","MP Number","MI Number","PR Reference","PO Reference","BAPP Number","BAST Number","GR Status"
        ];

        $headers = [
            "status","name","docstatus","end_date","site_id","return_notes","start_date","ts_submitted_vendor",
            "mitra_om","tower_name","sow","fullname_engineer","ts_ready_to_execute","timestamp_end","ts_approved_vendor",
            "timestamp_start","ts_open","ts_need_assign","ts_approved_sm","ts_expired","mo_reference","pdf_link",
            "regional","pid","tenant_id","customer","engineer","district_or_city","area","batch","year","funcloc",
            "site_id_tenant","mo_type","type","visitor_permit","province","address","maintenance_zone","lat_long",
            "kode_tipe_site","portofolio","return_schedule_notes","ts_request_schedule","order_number","mp_number",
            "mi_number","pr_reference","po_reference","bapp_number","bast_number","gr_status"
        ];
